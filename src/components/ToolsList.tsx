...
export default function ToolsList() {
  ...
  const getToolIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Workflow': return <Network className="w-5 h-5 text-blue-600" />;
      case 'Database': return <Database className="w-5 h-5 text-blue-600" />;
      case 'Code': return <Code className="w-5 h-5 text-blue-600" />;
      case 'Palette': return <Palette className="w-5 h-5 text-blue-600" />;
    ...
    if (action === 'enter') {
      ...
      if (progressFill) {
        gsap.to(progressFill, {
          backgroundColor: '#2563eb', // make fill brighter blue
          duration: 0.2
        });
      }
    } else {
  ...
  return (
    <section ref={containerRef} id="herramientas" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 scroll-mt-24">
      ...
      <div className="absolute bottom-10 right-10 w-96 h-96 bg-radial from-blue-100/30 to-transparent -z-10 pointer-events-none rounded-full blur-3xl" />

      {/* Title Panel */}
      <div className="text-center max-w-3xl mx-auto mb-16">
        <span className="text-xs uppercase font-mono tracking-widest text-slate-500 font-semibold mb-2 block flex items-center justify-center gap-1.5">
        ...
        <h2 className="font-display font-extrabold text-slate-900 text-3xl sm:text-4xl tracking-tight">
          Herramientas que he aprendido <span className="text-blue-600">en el Diplomado</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {TOOLS_LEARNED.map((tool, idx) => (
          <div
            ...
            className="tool-card glass-card p-6 rounded-2xl flex flex-col justify-between border border-white hover:cursor-pointer"
          >
            <div className="space-y-4">
              ...
              <div className="flex items-start justify-between">
                ...
                <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider bg-slate-100 px-2.5 py-0.5 rounded-full">
                  {tool.category}
                </span>
              </div>
            ...
            <div className="pt-5 border-t border-slate-100/60 mt-5 space-y-2">
              <div className="flex items-center justify-between text-xs font-medium text-slate-500">
                <span>Nivel de Dominio</span>
                <span className="font-mono text-blue-600 font-bold">{tool.proficiency}%</span>
              </div>
